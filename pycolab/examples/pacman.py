...
class PatrollerSprite(prefab_sprites.MazeWalker):
  ...
  def __init__(self, corner, position, character, virtual_position):
    ...
    if(bool(ord(character) % 2)):
        self.direction = self._WEST
    else:
        self.direction = self._EAST
    self._MOTIONS = (self._NORTH,self._WEST,self._SOUTH,self._EAST)
    self._VALID = []

  def direction_change(self):
     rand = random.randint(0,len(self._VALID)-1)
     
     # if we have more than 2 actions we pick a new random course
     # if we have two we continue
     # if we reach a dead end we pick random action
     
     if(len(self._VALID) > 2):
         return self._VALID[rand]
     elif(self.direction in self._VALID):
...
#        pattern_row, pattern_col+(1 if self._moving_east else -1)]
    ...
    for i in range(len(self._MOTIONS)):
        if self._check_motion(board, self._MOTIONS[i]) == None:
            self._VALID.append(self._MOTIONS[i])
    
    # Chooses whether to change direction or stay on course
    self.direction = self.direction_change()
    
    #based on the direction of travel executes proper move
    if(self.direction == self._EAST):self._east(board, the_plot)
    if(self.direction == self._NORTH):self._north(board, the_plot)
    if(self.direction == self._WEST):self._west(board, the_plot)
    if(self.direction == self._SOUTH):self._south(board, the_plot)
